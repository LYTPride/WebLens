...
- **关联资源**：对 Involved Object 提供跳转入口时，会切换到 **侧栏已支持的列表视图**（如 Pods、Deployments、StatefulSets、ConfigMaps、PVC、Services、Ingress、Nodes），并带上名称过滤，便于连贯排障。
- **v1 未在侧栏开放的资源类型**（如 DaemonSet、Job、CronJob、Secret 等）**不展示跳转**，避免进入无列表页；会话恢复时若 localStorage 中仍为上述隐藏视图键，会 **回落到 Pods**（实现见 `web/src/utils/v1HiddenViews.ts`）。
