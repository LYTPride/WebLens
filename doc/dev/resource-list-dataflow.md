...
| Events 列表表格与列/Involved 派生 | `web/src/components/EventsListTable.tsx`、`web/src/utils/eventTable.ts` |
| Events 默认/列排序比较 | `web/src/utils/resourceListSort.ts`（`compareEventsDefaultTriage`、`compareEventsForSort`） |
| Events Watch 缺口合并编排 | `web/src/pages/App.tsx` → `runEventsWatchGapFill` |
