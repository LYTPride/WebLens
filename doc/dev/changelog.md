...
- 用户表新增增量 `is_root` 标记；现有精确 `admin` 账号迁移为唯一根管理员。Store 策略和 SQLite 触发器共同阻止 root 改名、降级、禁用、删除或通过用户管理接口重置密码。
- 根管理员和平台管理员均可创建平台管理员/普通用户并管理其他非 root、非当前账号；删除继续要求先禁用。平台管理员只在已添加作用域中获得 operator，未添加命名空间与 Nodes 等集群级资源不再走 admin fallback。
- 新建和重置用户改为密码学随机一次性临时密码，接口只在本次响应返回明文；登录后强制设置永久密码，禁用/重置会撤销全部会话。
- 新增本机 `weblens-server reset-admin-password` 子命令及交互式 `scripts/reset-admin-password.sh` 包装脚本；恢复事务同时更新 bcrypt 哈希、强制改密、撤销 root 会话并写入 `root.password.recovery` 审计，部署包包含该脚本。
- 用户管理、本人改密和本机恢复审计补充目标用户与操作明细；Watch/Shell 每约 5 秒复核 session，Shell 同时复核 `pod.exec`，权限或会话撤销后主动断开。
- 权限配置界面可选择创建普通用户或平台管理员，高权限创建二次确认；root/当前账号管理操作明确禁用；临时密码支持复制，审计列表展示 detail 并可筛选密码相关动作。

## 2026-08-12

### 审计功能完善

- 资源删除、重启、扩缩容、YAML/配置保存和已有批量高危操作新增操作日志必填步骤，填写后继续使用原资源确认弹窗；取消任一步均不发出请求。
- 后端对全部资源写请求统一强制校验操作日志，覆盖 admin 与普通写权限用户，拒绝空值或超过 500 字符的日志，防止绕过前端。
- 审计模型新增 `operation_log` 增量迁移，资源操作成功或失败后随审计记录落库；管理员审计表新增“日志/查看”入口。
- 审计记录停止采集和返回来源 IP，管理员审计表同步移除“来源 IP”列。

### Permissions V2 管理界面优化

- 右上角盾牌菜单改为“用户管理 / 角色授权 / 作用域分组 / 审计记录”四个入口，与权限配置弹窗页签一一对应并可直接打开目标页。
- 默认密码复制增加 `navigator.clipboard` 失败后的 `execCommand` 兼容回退，修复 HTTP 部署或浏览器策略限制下重置密码后无法复制的问题。
- 角色授权保留用户主从选择，并把组授权与单独作用域授权拆成两个带标题、说明、计数、搜索和独立滚动的面板。
- 作用域分组改为可搜索分组列表与详情工作区；新建流程独立展示，分组信息、保存/删除和作用域成员列表分层呈现。
- 用户、授权用户、作用域组授权、单独作用域授权、作用域分组及分组内作用域列表均支持关键词即时过滤。
- 补齐盾牌菜单入口之间的分隔线，末项明确无底边并用行背景替代浏览器默认黑色焦点轮廓；默认密码提示切换配置页签后立即清理；审计结果状态胶囊固定为单行展示，避免状态码错行。

## 2026-08-05

### Permissions V2：角色、作用域分组与审计

- 保留现有 `admin/user` 平台身份，为普通用户新增 `viewer/operator` 作用域角色；旧版直接授权自动迁移为 operator。
- 新增扁平作用域分组、分组授权与单独作用域授权，重叠授权按 `operator > viewer` 合并。
- 后端按资源读取、资源写入、Logs、Shell、文件读取和文件写入能力拦截请求，并对批量 ConfigMap 操作逐作用域校验。
- 新增用户管理、角色授权、作用域分组、审计记录四个管理功能区；观察者界面隐藏写操作，YAML 使用只读模式。
- 活动 Shell 定期复核 exec 能力，权限撤销后主动断开；重要操作和权限拒绝写入结构化审计日志。

## 2026-07

### 认证入口视觉升级与权限配置交互

- **登录 / 首次改密页视觉**：认证入口改为固定深色双栏布局，移除登录页与首次改密页的 logo 和主题切换；左侧视频 Hero 保留 WebLens 品牌和三行居中文案，去掉注册式步骤；视频改为同源静态资源，避免客户端无法访问外部 CDN 时背景缺失，并使用固定视频滤镜加 5.2s 遮罩透明度呼吸，消除 Firefox 中视频层抖动和闪烁。
