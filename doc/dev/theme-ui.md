...
- **视频加载与舒适度**：登录背景视频作为 Vite 静态资源随 WebLens 同源发布，避免客户端无法访问外部 CDN 时背景缺失。`wl-auth-hero-video` 只使用固定滤镜与缩放；独立遮罩层通过 `wl-auth-video-breathe` 做 5.2 秒透明度呼吸，避免 Firefox 对实时视频滤镜和缩放逐帧重绘造成抖动或闪烁。
